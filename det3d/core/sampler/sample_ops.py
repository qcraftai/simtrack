        ...
        if len(sampled) > 0:
            ...
            for info in sampled:
                ...
                if "rot_transform" in info:
                        ...
                        s_points[:, :3] = box_np_ops.yaw_rotation(s_points[:, :4], rot, axis=2)
                
                s_points[:, :3] += info["box3d_lidar"][:3] 
                    
                s_points_list.append(s_points)
       
            if self._gt_drop_rate > 0:
            ...
            sampled_gt_boxes[:, 6:8] = 0 
            ret = {
                "gt_names": np.array([s["name"] for s in sampled])[mask2keep],
                "difficulty": np.array([s["difficulty"] for s in sampled])[mask2keep],
                "gt_boxes": sampled_gt_boxes[mask2keep].astype(np.float32),
